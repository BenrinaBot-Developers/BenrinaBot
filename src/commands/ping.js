...
module.exports = async function* (_details) {
  ...
  if (_details) {
    results.set("API").set("DATABASE");
    {
      ...
      await Promise.all([
        API.connectSunaba({ content: "init" })
          .then(() => values.set("Sunaba", new Date() - startsAt))
          .catch(console.log),
    ...
    async function db_ping() {
      ...
      console.log(body);
      values.set("Upstream", body?.result?.timestamp - startsAt).set("Downstream", endsAt - body?.response_timestamp);
