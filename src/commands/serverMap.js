...
const { Util, MessageAttachment, Permissions } = require("discord.js");
...
module.exports = async function({
  ...
  if (_showThreads === "all") {
    await Promise.all(this.from.guild.channels.cache.map((_channel) => {
  ...
  const isAccessible = (_channel) => {
    ...
    return _filteringByPermissions
  ...
  //Send
  if (_format === "file") return { files: [new MessageAttachment(Buffer.from(serverMap, 'utf-8'), "ServerMap.txt")] };
  else return Util.splitMessage(serverMap);
};