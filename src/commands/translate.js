...
module.exports.execute = async function(_text, _to, _from) {  //アロー関数は不可
...
module.exports.getLanguages = async function(_maxLength = 25) {
  const languageCodes = await fs.readFile(`${__dirname}/../assets/languageCodes.csv`, "utf-8")
  const data = await parse(languageCodes, { columns: true, ltrim: true, rtrim: true, escape: '\\' })
  const languages = data.sort((a, b) => {
      ...
      .map(_datum => {return {name:`${_datum.ja_name}/${_datum.en_name}`, value:_datum.code}})
  return languages;
};