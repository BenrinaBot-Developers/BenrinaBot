...
module.exports.search = async function({word:_word, limit:_limit, type:_type, language:_language = "ja"}){
  ...
  let title = `『${_word}』`
  let optionsString = [];
  if(_limit) {parameters.srlimit = _limit; optionsString.push(`limit:${_limit}`);}
  if(_type) {parameters.srwhat = _type; optionsString.push(`type:${_type}`);}
  ...
  const pageController = new PageController({title: title, pageLoop:true, pageNumber: true, authors:[this.from?.user.id]})
  const response = await API.searchWikipedia(parameters, _language)
    .catch(console.error);
    ...
    if(response?.query?.searchinfo?.totalhits){
      response.query?.search.forEach(_data => {
        ...
        //console.log(content)
        pageController.addPages({embeds: [
          functions.getEmbed(0x00ff00, `Wikipedia : ${_data.title}`, `...${content}...`)
            .setURL(`https://${_language}.wikipedia.org/wiki/${encodeURIComponent(_data.title)}`)
            .setTimestamp(new Date(_data.timestamp))
        ]});
      })
    } else {
      pageController.addPages({embeds: [functions.getEmbed(0x00ff00, "Wikipedia", "Couldn't Find")]});
    }
