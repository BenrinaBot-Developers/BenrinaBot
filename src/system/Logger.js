...
const { Discord, Collection, Util } = require("discord.js");
...
const log = async (type, {date, command, method, message, error , data, response:{config:apiConfig, data:apiResponse} = {}}) => {
  let channnelIDs;
  let information = new Collection();
  switch(type) {
    case Type.START_UP:
      channnelIDs = systemLogChannels.start_up;
      information
        .set("StartsAt", getTime(date));
      break;

    case Type.ERROR:
      channnelIDs = systemLogChannels.error;
    ...
    case Type.COMMAND:
      channnelIDs = systemLogChannels.command;
      information
        .set("Command", command)
        .set(Symbol("$"))
    ...
    case Type.API:
      channnelIDs = systemLogChannels.api;
      const replacers = {
        response(key, value) {
          if(!key) return value;
          return String(value);
      ...
      else if(apiResponse) information.set("Response", codeBlock("json", JSON.stringify(apiResponse, replacers.response, 2)));
      break;

    case Type.DATABASE:
      channnelIDs = systemLogChannels.database
  ...
  let infomationString = "", infomationStringForDev = "";
  information.forEach((_content, _tag) => {
    //console.log(typeof _tag)
    if(typeof _tag === "symbol") {
      if(_tag?.description === "$") infomationString += "\n";
      else infomationString += `${_tag?.description}\n`;
    } else {
      infomationString += `${_tag}: ${_content}\n`;
    }
  })
  //console.log(response.data, information, infomationString, channnelIDs);
  let channnelTags = [];
  const sentChannelIDs = [];
  //const title = type === Type.ERROR && error?.type ? error.type : type.description;
  const title = type.description;
  channnelIDs.forEach(_chID => {
    //console.log(_chID)
    channnelTags.push(`<#${_chID}>`);
    sentChannelIDs.push(functions.send(_chID, {embeds:[
      functions.getEmbed(0x00ff00, title , infomationString)
    ]}).catch(console.log));
  });
  information = information.filter((_, _key) => typeof _key !== "symbol")
  
  channnelTags = channnelTags.join(" ");
  infomationString = Util.removeMentions(infomationString.replace("\n", " "));
  systemLogChannels.synthesized.forEach(_chID => {
    sentChannelIDs.push(functions.send(_chID,
      `${channnelTags}：${title} <${[...information.keys()].join(", ")}>  [${getTime()}]`
    ).catch(console.log));
  });
